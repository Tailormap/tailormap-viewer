  ...
  public addDrawingLegend(doc: jsPDF, _width: number, _height: number): Observable<void> {
    return this.store$.select(selectDrawingFeatures).pipe(
      ...
      concatMap(features => {
        ...
        //const totalHeight = startY + (features.length * (iconSize - 2.5) + iconSize / 2) - (fontSize / 2);
        ...
        return from(features).pipe(
          concatMap((feature, idx) => {
            const columnX = idx >= maxItemsPerColumn ? 120 : 0;
            const rowIdx = idx >= maxItemsPerColumn ? idx - maxItemsPerColumn : idx;

            const y = startY + (rowIdx * (iconSize - 2.5) + iconSize / 2) - (fontSize / 2);
            doc.setFontSize(fontSize);
            doc.setLineWidth(rowWidth);
            const text: string[] = doc.splitTextToSize(feature.attributes.style.description || '', rowWidth);
              ...
              return this.httpClient.get(TailormapApiConstants.BASE_URL + feature.attributes.style.markerImage, { responseType: 'text' }).pipe(
                ...
                concatMap(svgContent => {
                  ...
                  const options: Svg2pdfOptions = {
                    x: startX + columnX,
                    y: y, // slight adjustment so icon and text align
                    width: iconSize - 4.5,
                    height: iconSize - 4.5,
