...
describe('AttributeListStatisticsService', () => {
  ...
  describe('loadStatistics', () => {
    ...
    it('loading -> success: should set isLoading=true then resolve with value after success', () => {
      ...
      // Since all observables (mock store + of()) are synchronous, loading+resolution happens in same tick
      const loadingEmission = emissions.find(e => e.some(s => s.isLoading));
      const resolvedEmission = emissions[emissions.length - 1];

      expect(loadingEmission).toBeDefined();
    ...
    it('loading -> error: should set hasError=true and value=0 when loading fails', () => {
      ...
      expect(resolvedEmission[0].value).toBe(0);
    });
